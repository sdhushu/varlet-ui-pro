...
const fixedBottom: ComputedRef<boolean> = computed(() => proProps.fixedBottom)

let active = $ref(proProps.active)

const tabList: ComputedRef<Array<Record<string, string>> | unknown> = computed(() => proProps.tabList)

const onTabClick = (tab: Record<string, string>, index: number) => {
  const currentActive = tab.name ?? index
  const { active, onChange, onClick } = proProps
  proProps["onUpdate:active"]?.(currentActive)
