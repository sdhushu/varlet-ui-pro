...
const stringContents: ComputedRef<boolean> = computed(
...
<template>
  <var-cell class="pro-notice-card">
    <template v-if="props.noticeIcon" #icon>
      <var-icon :name="props.noticeIcon" />
    </template>
    <template #default>
      <div class="pro-notice-card__content">
        <div v-if="stringContents" class="var--ellipsis">
          {{ props.contents }}
        </div>
        <var-swipe
          v-else
          class="pro-notice-card__swipe"
          :loop="props.loop"
          vertical
    ...
    <template #extra>
      <div class="pro-notice-card__extra">
        <slot name="extra" />
        <var-icon v-if="props.arrow" name="chevron-right" />
      </div>
    </template>
  </var-cell>
</template>
